"""
Trading Economics API ORM models.
"""
...
class TradingEconomicsSyncStatus(str, enum.Enum):
    IDLE = "idle"
    SYNCING = "syncing"
    SUCCESS = "success"
    FAILED = "failed"
...
class TradingEconomicsApiConfig(Base):
    __tablename__ = "trading_economics_api_config"

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    provider_name: Mapped[str] = mapped_column(
        String(120), nullable=False, default="Trading Economics"
    )
    api_key: Mapped[str | None] = mapped_column(Text, nullable=True)
    base_url: Mapped[str] = mapped_column(
        Text, nullable=False, default="https://api.tradingeconomics.com"
    )
    ...
    sync_status: Mapped[TradingEconomicsSyncStatus] = mapped_column(
        pg_enum(TradingEconomicsSyncStatus),
        nullable=False,
        default=TradingEconomicsSyncStatus.IDLE,
    )
...
class TradingEconomicsApiLog(Base):
    __tablename__ = "trading_economics_api_logs"
    ...
    config_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("trading_economics_api_config.id", ondelete="SET NULL"),
        nullable=True,
    )
...
class TradingEconomicsCountryData(Base):
    __tablename__ = "trading_economics_country_data"
    __table_args__ = (
        UniqueConstraint(
            "country_code",
            "data_year",
    ...
    te_country_slug: Mapped[str] = mapped_column(String(120), nullable=False, index=True)
