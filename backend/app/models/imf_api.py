...
class ImfSyncStatus(str, enum.Enum):
    IDLE = "idle"
    SYNCING = "syncing"
    SUCCESS = "success"
    FAILED = "failed"
...
class ImfApiConfig(Base):
    __tablename__ = "imf_api_config"

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    provider_name: Mapped[str] = mapped_column(
        String(120), nullable=False, default="IMF DataMapper"
    )
    api_key: Mapped[str | None] = mapped_column(Text, nullable=True)
    base_url: Mapped[str] = mapped_column(
        Text, nullable=False, default="https://www.imf.org/external/datamapper/api/v1"
    )
    ...
    sync_status: Mapped[ImfSyncStatus] = mapped_column(
        pg_enum(ImfSyncStatus), nullable=False, default=ImfSyncStatus.IDLE
    )
...
class ImfApiLog(Base):
    __tablename__ = "imf_api_logs"

    id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), primary_key=True, default=uuid.uuid4
    )
    config_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("imf_api_config.id", ondelete="SET NULL"), nullable=True
    )
...
class ImfCountryData(Base):
    __tablename__ = "imf_country_data"
    __table_args__ = (
        UniqueConstraint("country_code", "data_year", name="uq_imf_country_data_code_year"),
    )
    ...
    imf_country_code: Mapped[str] = mapped_column(String(10), nullable=False, index=True)
