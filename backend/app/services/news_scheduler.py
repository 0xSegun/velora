"""Background scheduler for automatic news API synchronization."""
...
from app.services.news_service import get_config, sync_news

logger = logging.getLogger(__name__)

_scheduler_task: asyncio.Task | None = None
_CHECK_INTERVAL_SECONDS = 120
_BACKOFF_BASE_SECONDS = 180
...
async def _scheduler_loop() -> None:
    ...
    logger.info("News API scheduler started")
    while True:
        try:
            ...
            async with async_session_factory() as db:
                ...
                if (
                    ...
                    and config.sync_status != NewsSyncStatus.SYNCING
                    ...
                    if result.get("success"):
                        _consecutive_failures = 0
                        _backoff_until = None
                        logger.info("News scheduled sync: %s", result.get("message"))
                    else:
                        ...
                        logger.warning("News sync failed: %s", result.get("message"))
        except Exception as exc:
            logger.exception("News scheduler error: %s", exc)
        await asyncio.sleep(_CHECK_INTERVAL_SECONDS)


def start_news_scheduler() -> asyncio.Task:
...
async def stop_news_scheduler() -> None:
    global _scheduler_task
    if _scheduler_task and not _scheduler_task.done():
